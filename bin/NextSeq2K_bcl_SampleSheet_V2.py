...
parser.add_argument('--novaSeq', default="False", help='order type, RNAseq or 10x ?')
...
# use AAC7NV3HV , /n/ngs/data/NextSeq2K/230919_VH00629_133_AAC7NV3HV as test 
    ...
    with open(os.path.join(args.output_dir,"SampleSheet.csv"), "w") as f:
        ...
        for index_, row in df.iterrows():
            ...
            if len(data_index_list) == 2:
                if args.novaSeq != "False":
                    # index = ','.join(str(Seq(s['sequence']).reverse_complement()) for s in data_index_list)  # This reverse complement both index1 and 2. Can be used for NextSeq500
                    index = ','.join([data_index_list[0]['sequence'], str(Seq(data_index_list[1]['sequence']).reverse_complement())]) # This reverse complement only index2. Can be used for NovaSeq 
                else:
